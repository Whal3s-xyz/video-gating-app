...
export default function Steps({steps, currentStep, progress}: {
    steps: StepsProps[],
    currentStep: number,
    progress?: string
}) {
            ...
            <ol role="list" className="overflow-hidden">
                {steps.map((step, stepIdx) => (
                  ...
                  <span className="flex h-9 items-center">
                    ...
                    <span className="text-sm font-medium">{step.nameUpcoming}</span>
                  </span>
                                </span>
                            </>
                        ) : stepIdx === currentStep ? (
                  ...
                  <span className="flex h-9 items-center" aria-hidden="true">
                    ...
                    <span className="text-sm font-medium text-indigo-600">{step.name}</span>
                                        {progress && <span className="text-sm text-gray-500">{progress}</span>}                  </span>
                                </span>
                            </>
                        ) : (
                            <>
                  ...
                  <span className="flex h-9 items-center" aria-hidden="true">
                    <span
                      ...
                      <span className="h-2.5 w-2.5 rounded-full bg-transparent group-hover:bg-gray-300"/>
                                    ...
                                    <span className="ml-4 flex min-w-0 flex-col">
                    <span className="text-sm font-medium text-gray-500">{step.nameCompleted}</span>
